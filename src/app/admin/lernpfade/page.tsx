  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <p className="mt-1 text-sm text-foreground-soft">
        Gruppiere Videos in einer festen Reihenfolge, z. B. für den Einstieg in ein Thema.
      </p>

      <LernpfadErstellenForm />

      {liste.length === 0 ? (
        <EmptyState icon="🧭" text="Noch keine Lernpfade angelegt." />
      ) : (
          ...
          {liste.map((lp) => (
            ...
            >
              ...
              <p className="mt-2 font-mono text-xs text-blueprint">
                {lp.lernpfad_videos[0]?.count ?? 0} Videos – bearbeiten →
              </p>
            </Link>
          ))}
        </div>
